...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            ...
            String[] stylishColumns = new String[length];
            int index = 0;
            while (index < selectors.size()) {
                stylishColumns[index] = selectors.get(index);
                index++;
            }
            table = new Table(stylishColumns);
            _input.next(")");
            return table;
        } else {
            _input.next("as");
    ...
    Table selectClause() {
        ...
        columnAdder(columnNames);
        _input.next("from");
        Table t1 = tableName();
        if (_input.nextIs(";")) {
            output = t1.select(columnNames, conditions);
        }
        if (_input.nextIf(",")) {
            Table t2 = tableName();
            if (_input.nextIs("where")) {
                conditions = conditionClause(t1, t2);
                output = t1.select(t2, columnNames, conditions);
            } else {
                output = t1.select(t2, columnNames, conditions);
            }
        }
        if (_input.nextIs("where")) {
            return t1.select(columnNames, conditionClause(t1));
        }
        return output;
    }

    /**Adds all COLUMNNAMES into an ArrayList for further processing.**/
    void columnAdder(ArrayList<String> columnNames) {
        while (_input.nextIf(",")) {
            columnNames.add(columnName());
        }
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        conditionAdder(conditions, tables);
        return conditions;
    }
    /** Adds all the necessary conditions.
     *  into an array for further processing.
     * CONDITIONS are all the conditions that need to be
     * satisfied and TABLES are where the CONDITIONS are tested.**/
    void conditionAdder(ArrayList<Condition> conditions, Table...tables) {
